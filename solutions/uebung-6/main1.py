...
def wurzelApprox(x, eps):
    ...
    """
    r = x / 2
    # Bemerkung: die wahre Wurzel von x liegt zwischen r und x/r
    while abs(r - x / r) >= eps:
        r = (r + x / r) / 2
...
if __name__ == "__main__":
    ...
    print(
        f"Die Anzahl Matrix-Elemente auf oder oberhalb der Diagonalen beträgt für eine 20x20 Matrix {anzahlPaare(20)+20} \n"
    )
    ...
    print(
        f"Die Summe aller Beträge der Eigenwerte der Matrix \n {A} \n ist {summeAbsEigval(A)} \n"
    )
    ...
    print(
        f"Die Wurzel von 10 ist auf 4 Nachkommastellen genau {wurzelApprox(10,10**-4):.4f}"
    )

    print(
        f"Die Wurzel von 2 ist auf 8 Nachkommastellen genau {wurzelApprox(2,10**-8):.8f} \n"
    )

    eps = 10**-4
    C = np.array(
        [
            [wurzelApprox(2, eps), wurzelApprox(3, eps)],
            [wurzelApprox(5, eps), wurzelApprox(6, eps)],
        ]
    )
    print(
